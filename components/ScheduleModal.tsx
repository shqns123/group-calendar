...
import { X, Plus, Trash2, Bell, ToggleLeft, ToggleRight } from "lucide-react";
...
export default function ScheduleModal({ groupId, groupName, onClose }: Props) {
  ...
  // 새 스케줄 입력 상태
  const [message, setMessage] = useState("특근 가능여부 체크 해주세요");
  const [selectedDays, setSelectedDays] = useState<number[]>([3]); // 기본: 수요일
                ...
                {DAY_LABELS.map((label, idx) => (
                  <button
                    ...
                    style={{
                      ...
                      border: "1px solid",
                      borderColor: selectedDays.includes(idx) ? "var(--text-primary)" : "var(--border)",
                      background: selectedDays.includes(idx) ? "var(--text-primary)" : "transparent",
                      color: selectedDays.includes(idx) ? "var(--surface)" : "var(--text-secondary)",
