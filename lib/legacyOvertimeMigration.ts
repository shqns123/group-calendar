...
function hasUtcMidnight(date: Date): boolean {
  return (
    date.getUTCHours() === 0 &&
...
export function normalizeLegacyOvertimeDate(date: Date): Date {
  return new Date(`${getSeoulDateKey(date)}T00:00:00.000Z`);
}

export function isLegacyOvertimeMigrationCandidate(event: LegacyOvertimeShape): boolean {
